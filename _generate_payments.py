...
legend = 'For dates between ' + first_day_of_prev_month.strftime('%Y-%m-%d') + ' and ' + today.strftime('%Y-%m-%d')
...
with open(csv_file, 'w', newline='') as csvFile:
	...
	writer.writerow([])	
	writer.writerow(["panel_id", "Treatment #", "Reward for Linking ($)", "Reward for keeping account active ($)", "New accounts linked", "Total Account", "Accounts Active", "Total Payment ($)"])
	for user in users:
		items = Fin_Items.objects.filter(user_id=user)
		if items:
			treatment = fetch_treatment(user.pk)
			new_accounts_in_last_month = Fin_Items.objects.filter(user_id=user, date_created__gte=first_day_of_prev_month, deleted=0).count()
			...
			writer.writerow([user.username, treatment.treatment, treatment.reward_link, treatment.reward_monthly, new_accounts_in_last_month, total_accounts, active_accounts, payment])
			payment = 0
